...
source('mplus.r')  # to generate plotss
library(sp)
library(knitr)
...
# Multiple-indicator
## High Reliability ############################################################
### Correct Model Fitting (Data with ME ==> Fit DFVAR) #########################
####1. N=200, T=60
```{r, include=FALSE}
...
lsigmas = c(log(1.2)) 
S <- diag(rep(exp(lsigmas),nf)) 
#R_values <- c(0.00001) 
R <- diag(rep(1, nf), nrow=nf) 
Sigma_VAR <- S %*% R %*% S
```
...
nt = ntt + discard  #@@@@
condition = 3  #ny_perf = one indicator per latent variable f.
####################### population psi.mu.sqrd (intercept var) #################
sigmaInter = sqrt(2.5) # low.rel=sqrt(4), high.rel=sqrt(2.5)
...
Sigma_omega = diag(rep(.4, ny), nrow=ny) #low.rel=3, high.rel=.4
####################### Condition box end ######################################
```
...
############
  for(rp in 1:reps){
    dir.create(paste0('/Users/hyungeun/R/M/3N200T60H_',rp,'/')) 
...
dim(f);dim(y);dim(muy);dim(muInter);dim(intercept);dim(muAR);dim(AR);dim(bmu);dim(b)      
...
save(np, ntt, nx, nf, ny_perf, ny, y, f,
       ...
       file=paste0('/Users/hyungeun/R/M/3N200T60H_',rp,'/sim',rp,".Rdata"))

write.table(repdata, file=paste0('/Users/hyungeun/R/M/3N200T60H_',rp,'/sim',rp,".dat"), row.names = FALSE, col.names = FALSE, sep = "\t")
}

############## Data check ##############
f = f[,1:ntt,];f1 = f[,,1];f2 = f[,,2]
dim(f1);dim(f2)
y = y[,1:ntt,];y1 = y[,,1];y2 = y[,,2];y3 = y[,,3]
y4 = y[,,4];y5 = y[,,5];y6 = y[,,6]
dim(y1);dim(y2);dim(y3);dim(y4);dim(y5);dim(y6)

#contrast
y_range <- range(c(f1, f2, y1, y2)) #to make y-axis scales same
# f1 vs y1
